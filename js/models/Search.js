...
export default class Search {
  ...
  async searchForLocations() {
    try {
      const res = await fetch(
        `https://developers.zomato.com/api/v2.1/cities?q=${this.query}`,
        options
      );
  ...
  async searchRestaurants(
    ...
    try {
      const res = await fetch(
        `https://developers.zomato.com/api/v2.1/search?entity_id=${entity_id}&entity_type=${entity_type}&count=${count}&start=${start}`,
        options
      );
      const data = await res.json();
      // console.log(data);
