...
const searchCtrl = async (id = null, type = null, start = null) => {
  ...
  if (id !== null && type !== null && start !== null) {
    ...
    // ...animate old results...
    elements.results.classList.add('fetching');
    ...
    //... animate new results...
    elements.results.classList.remove('fetching');
    ...
    if (query) {
      ...
      try {
        ...
        if (state.search.cityMatches && state.search.cityMatches.length > 1) {
          ...
          searchView.renderRestaurantsList(
            state.search.searchDetails.restaurants
          );

          // ...along with pagination buttons if needed
          searchView.renderPaginationButtons(
            state.search.searchDetails,
            cityID
          );
          ...
          searchView.renderErrorMsg(
            'Tego miasta nie ma w naszej bazie danych, przepraszamy...'
          );
          searchView.hideLoader();
        }
      } catch (err) {
